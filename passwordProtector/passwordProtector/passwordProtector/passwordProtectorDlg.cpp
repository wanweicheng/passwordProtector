...
void CpasswordProtectorDlg::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnOK();
	MessageBox(_T("确认登录"),"提示框",NULL);
	UpdateData(true);
	if (_T("a362905343")  == userNameString && _T("82342754") == userPasswordString)
	{
		MessageBox(_T("登录成功"),"提示框",NULL);
	}
	else
		MessageBox(_T("登录失败"),"提示框",NULL);
}
